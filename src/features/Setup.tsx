...
import { Shield, User, Lock, Mail, Check, ArrowRight, Building, Cloud } from 'lucide-react';
	...
	const handleCreateAdmin = async () => {
		...
		try {
			await db.users.add({
				...
				syncId: googleAccessToken || undefined
			});
		...
		<div className="min-h-screen bg-white dark:bg-[#1a1c1e] flex items-center justify-center p-6">
			<div className="max-w-2xl w-full">
				...
				<Card className="p-8 shadow-xl border border-[#dadce0] dark:border-[#3c4043]">
					...
					{step === 2 && (
						<div className="space-y-6 animate-in slide-in-from-right-8">
							<Input label="Nombre Completo" name="fullName" value={formData.fullName} onChange={handleChange} leftIcon={<User size={18} />} placeholder="Ej: Juan Pérez" />
							<Input label="Usuario / Email" name="username" value={formData.username} onChange={handleChange} leftIcon={<Mail size={18} />} placeholder="admin@taller.com" />
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<Input label="Contraseña" type="password" name="password" value={formData.password} onChange={handleChange} leftIcon={<Lock size={18} />} />
								<Input label="Confirmar Contraseña" type="password" name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} leftIcon={<Lock size={18} />} />
							</div>

							<Button variant="primary" className="w-full h-12 mt-4" onClick={handleCreateAdmin} isLoading={isLoading} rightIcon={<ArrowRight size={18} />}>Crear Administrador</Button>
						</div>
					)}
					...
					{step === 3 && (
						<div className="space-y-8 animate-in slide-in-from-right-8 text-center">
							<div className="flex flex-col items-center justify-center gap-4 py-6">
								...
								<p className="text-sm text-[#5f6368] dark:text-[#9aa0a6] max-w-sm">Vincula tu cuenta de Google Drive para respaldar automáticamente tu base de datos y trabajar desde múltiples dispositivos.</p>
							</div>
							...
							{googleAccessToken ? (
								...
								<Button variant="outline" className="w-full max-w-sm mx-auto h-12 bg-white dark:bg-[#202124] text-[#3c4043] dark:text-white border-[#dadce0] dark:border-[#5f6368]" onClick={handleCloudLink}>
									<span className="mr-2">🔗</span> Vincular Google Account
								</Button>
							)}

							<div className="pt-8 border-t border-[#f1f3f4] dark:border-[#3c4043] flex gap-4">
								<Button variant="ghost" className="flex-1" onClick={finishSetup}>Saltar por ahora</Button>
								<Button variant="primary" className="flex-1" onClick={finishSetup} disabled={!googleAccessToken}>Finalizar Setup</Button>
							</div>
						</div>
					)}
