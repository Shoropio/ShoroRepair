      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-[#1a1c1e] p-6 lg:p-8 rounded-none shadow-xl shadow-blue-500/5 border border-[#f1f3f4] dark:border-white/5">
        ...
        <div className="flex items-center gap-3">
          <div className="flex bg-[#f1f3f4] dark:bg-white/5 p-1 rounded-none">
            <button onClick={() => setViewMode('grid')} className={`p-2 rounded-none transition-all ${viewMode === 'grid' ? 'bg-white dark:bg-[#1a1c1e] text-[#1a73e8] shadow-sm' : 'text-[#5f6368]'}`}><LayoutGrid size={16} /></button>
            <button onClick={() => setViewMode('list')} className={`p-2 rounded-none transition-all ${viewMode === 'list' ? 'bg-white dark:bg-[#1a1c1e] text-[#1a73e8] shadow-sm' : 'text-[#5f6368]'}`}><List size={16} /></button>
          </div>
          {hasPermission('canManageClients') && (
            <Button
              variant="primary"
              className="rounded-none px-4 lg:px-6 py-2.5 font-bold uppercase tracking-widest text-[10px]"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
        <Card className="p-5 border-none shadow-lg shadow-blue-500/5">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-blue-50 dark:bg-blue-900/10 text-blue-600 rounded-none">
              <Users size={20} />
            </div>
            <div>
        ...
        <Card className="p-5 border-none shadow-lg shadow-emerald-500/5">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-emerald-50 dark:bg-emerald-900/10 text-emerald-600 rounded-none">
              <History size={20} />
            </div>
            <div>
        ...
        <Card className="p-5 border-none shadow-lg shadow-indigo-500/5">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-indigo-50 dark:bg-indigo-900/10 text-indigo-600 rounded-none">
              <TrendingUp size={20} />
            </div>
            <div>
                  ...
                  <tr key={client.id} className="hover:bg-blue-50/20 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex justify-end gap-1.5 opacity-25 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => openEdit(client)} className="p-2 bg-blue-50 text-blue-600 rounded-none"><Edit2 size={14} /></button>
                        <button className="p-2 bg-emerald-50 text-emerald-600 rounded-none"><MessageSquare size={14} /></button>
                        <button onClick={() => handleDelete(client.id!)} className="p-2 text-red-500 hover:bg-red-50 rounded-none"><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
