  ...
  return (
    <div className="space-y-8 animate-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card variant="tonal" className="p-4 flex items-center justify-between">
          <div className="space-y-1">
            ...
            <h3 className="text-2xl font-semibold text-[#8b5cf6]">{stats.pending}</h3>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2" header={<h3 className="text-sm font-bold text-[#3c4043] dark:text-white">{t('dashboard.workshop_status')}</h3>}>
          <div className="h-[300px]">
            <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={0}>
              <BarChart data={stats.statusGroups}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f3f4" />
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#5f6368', fontSize: 11 }} />
                <YAxis axisLine={false} tickLine={false} tick={{ fill: '#5f6368', fontSize: 11 }} />
                <Tooltip cursor={{ fill: '#f8f9fa' }} contentStyle={{ border: 'none', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
        ...
        <Card header={<h3 className="text-sm font-bold text-[#3c4043] dark:text-white">{t('dashboard.recent_entries')}</h3>} footer={<Link to="/orders" className="text-xs font-bold text-[#1a73e8] hover:underline flex items-center gap-1 justify-center">{t('dashboard.view_all')} <ArrowRight size={14} /></Link>}>
