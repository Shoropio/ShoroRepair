...
const UsersPage: React.FC = () => {
	...
	const handleSave = async (e: React.FormEvent) => {
		...
		try {
			...
			if (existingUser && (!editingUser || existingUser.id !== editingUser.id)) {
				toast.error("Este identificador de usuario ya se encuentra registrado");
				return;
			}
			...
			if (editingUser) {
				...
				toast.success("Nuevo operador registrado");
			}
			closeModal();
		} catch (error) {
			toast.error("Fallo al sincronizar con base de datos");
		}
	};
	...
	const toggleStatus = async (user: AppUser) => {
		...
		toast.success(`Acceso de operador ${!user.active ? 'Habilitado' : 'Suspendido'}`);
	};

	const handleDelete = async (user: AppUser) => {
		if (user.role === 'Admin') return;
		if (confirm(`¿Proceder con la baja definitiva de ${user.fullName}?`)) {
			await db.users.delete(user.id!);
			toast.success("Operador removido del sistema");
		}
	};

	if (!users) return <div className="p-20 text-center animate-pulse text-xs font-black text-gray-400 uppercase tracking-widest">Validando Registro de Operadores...</div>;
		...
		<div className="space-y-8 animate-in pb-20">
			...
			<header className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 bg-white dark:bg-[#1a1c1e] p-10 rounded-[2.5rem] shadow-xl shadow-blue-500/5 border border-[#f1f3f4] dark:border-white/5 relative overflow-hidden">
				<div className="relative z-10">
					...
					<p className="text-sm text-[#5f6368] dark:text-[#9aa0a6] mt-2 font-medium max-w-md">
						Gestión de accesos, roles técnicos y auditoría de personal para ShoroRepair.
					</p>
				</div>

				<div className="flex flex-wrap items-center gap-4 relative z-10">
					<Link to="/roles">
						<Button variant="outline" className="rounded-2xl px-6 py-4 font-black uppercase text-[10px] tracking-widest border-gray-200" leftIcon={<Settings2 size={18} />}>Configurar Roles</Button>
					</Link>
					<Button
						variant="primary"
			...
			<div className="flex flex-col md:flex-row gap-4">
				<div className="relative flex-1 group">
					...
					<input
						type="text"
						placeholder="Buscar por nombre o identificador de operador..."
						value={search}
				...
				<div className="flex bg-white dark:bg-[#1a1c1e] p-2 rounded-[1.5rem] shadow-xl shadow-black/5 border border-[#f1f3f4] dark:border-white/5">
					<button onClick={() => setViewMode('grid')} className={`px-5 py-2.5 rounded-xl transition-all flex items-center gap-2 text-xs font-bold uppercase tracking-wider ${viewMode === 'grid' ? 'bg-[#1a73e8] text-white' : 'text-[#5f6368]'}`}><LayoutGrid size={16} /> Grid</button>
					<button onClick={() => setViewMode('list')} className={`px-5 py-2.5 rounded-xl transition-all flex items-center gap-2 text-xs font-bold uppercase tracking-wider ${viewMode === 'list' ? 'bg-[#1a73e8] text-white' : 'text-[#5f6368]'}`}><List size={16} /> Lista</button>
				</div>
			</div>
					...
					{users.map(user => (
						<Card key={user.id} className={`group p-8 rounded-[2.5rem] transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 relative overflow-hidden border-[#f1f3f4] dark:border-white/5 bg-white ${!user.active ? 'opacity-60 grayscale' : ''}`}>
							...
							<div className="flex items-center justify-between pt-6 border-t border-gray-100">
								<Badge variant={user.role === 'Admin' ? 'brand' : user.role === 'Technician' ? 'warning' : 'success'} size="xs" className="px-4 py-1 font-black uppercase tracking-widest">{user.role}</Badge>
								<button
						...
						<table className="w-full text-left">
							<thead>
								<tr className="bg-[#f8f9fa] dark:bg-white/[0.02] border-b border-[#f1f3f4] dark:border-white/5">
									<th className="px-8 py-5 text-[10px] font-black text-[#5f6368] uppercase tracking-[0.2em]">Identidad del Operador</th>
									<th className="px-8 py-5 text-[10px] font-black text-[#5f6368] uppercase tracking-[0.2em]">Rango / Acceso</th>
									<th className="px-8 py-5 text-[10px] font-black text-[#5f6368] uppercase tracking-[0.2em]">Estado Bio</th>
									<th className="px-8 py-5 text-[10px] font-black text-[#5f6368] uppercase tracking-[0.2em] text-right">Acciones</th>
								</tr>
							</thead>
							<tbody className="divide-y divide-[#f1f3f4] dark:divide-white/5">
								{users.map(user => (
									<tr key={user.id} className={`hover:bg-blue-50/20 transition-colors group ${!user.active ? 'opacity-40' : ''}`}>
										...
										<td className="px-8 py-6">
											<Badge variant={user.role === 'Admin' ? 'brand' : (user.role === 'Technician' ? 'warning' : 'success')} size="xs" className="px-4 font-black uppercase">{user.role}</Badge>
										</td>
										<td className="px-8 py-6">
											<div className="flex items-center gap-2">
												...
												<span className={`text-[10px] font-black uppercase tracking-widest ${user.active ? 'text-emerald-600' : 'text-red-600'}`}>
													{user.active ? 'Sincronizado' : 'Sin Acceso'}
												</span>
											</div>
										</td>
			...
			<Modal
				isOpen={showAdd}
				onClose={closeModal}
				title={editingUser ? "Control de Operador" : "Nuevo Ingreso Técnico"}
				subtitle="Registro Maestro de Seguridad ShoroRepair"
				size="2xl"
				footer={<div className="flex gap-4 px-8 pb-6"><Button variant="ghost" className="rounded-2xl px-8" onClick={closeModal}>Abortar</Button><Button variant="primary" className="rounded-2xl px-12 shadow-xl shadow-blue-500/20 font-black uppercase tracking-widest text-[11px]" onClick={handleSave}>Sincronizar Credenciales</Button></div>}
			>
				<form onSubmit={handleSave} className="space-y-8 py-6">
					<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
						<Input label="Nombre Completo del Operador" placeholder="Ej: Ing. Jorge Murillo" value={formData.fullName} onChange={v => setFormData({ ...formData, fullName: v.target.value })} required />
						<Input label="Identificador de Acceso (Usuario)" placeholder="Ej: jmurillo_tech" value={formData.username} onChange={v => setFormData({ ...formData, username: v.target.value })} required />
					</div>
					<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
						<Input label={editingUser ? "Nueva Contraseña (Opcional)" : "Contraseña de Seguridad"} type="password" value={formData.password} onChange={v => setFormData({ ...formData, password: v.target.value })} required={!editingUser} leftIcon={<Lock size={16} />} />
						<Select label="Rango de Seguridad" value={formData.role} onChange={v => setFormData({ ...formData, role: v.target.value as UserRole })}>
							<option value="Admin">Chief Administrator</option>
							<option value="Technician">Lab Technician</option>
							<option value="Receptionist">Front-Desk Agent</option>
						</Select>
					</div>
				</form>
			</Modal>
