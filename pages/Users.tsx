...
import { Card, Button, Input, Modal, Badge } from '../src/components';
                    ...
                    {users?.map(user => (
                        <Card key={user.id} variant="outlined" className="group overflow-hidden">
                            <div className="p-1 flex flex-col items-center text-center">
                                ...
                                {/* Avatar */}
                                <div className={`w-16 h-16 rounded-none flex items-center justify-center mb-4 transition-transform group-hover:scale-105 ${user.role === 'Admin' ? 'bg-[#e8f0fe] text-[#1a73e8]' :
                                    user.role === 'Technician' ? 'bg-[#fef7e0] text-[#f9ab00]' :
