...
import { compressImage } from '../src/utils/storageUtils';
import { cleanAllDuplicates } from '../src/utils/duplicateCleanup';
...
const Settings: React.FC = () => {
    ...
    const handleCloudBackup = async () => {
        toast.promise(uploadBackupToCloud(), {
            loading: 'Sincronizando con Drive...',
            success: () => {
                refreshCloudList();
    ...
    const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const { uploadImage } = await import('../src/utils/storageUtils');
    ...
    return (
        <div className="space-y-10 animate-in pb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                            ...
                            <div className="lg:col-span-3 space-y-4">
                                <div className="p-4 bg-[#fef7e0] dark:bg-[#fbbc04]/10 rounded-none border border-[#f9ab00]/20">
                                    <h5 className="text-xs font-bold text-[#f9ab00] flex items-center gap-2 mb-2 uppercase tracking-wide">
                                        <Info size={14} /> {t('settings.firebase_guide')}
                                    </h5>
                ...
                {user?.role === 'Admin' && (
                    <Card className="lg:col-span-2" header={<div className="flex items-center gap-2 font-bold text-sm text-[#3c4043] dark:text-white"><Database size={16} className="text-[#1a73e8]" /> {t('settings.database')}</div>}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-6">
                                <div className="bg-[#e8f0fe] dark:bg-[#1a73e8]/10 p-6 rounded-none flex items-start gap-4">
                                    <div className="p-3 bg-white dark:bg-[#1a1c1e] rounded-none shadow-sm"><HardDrive size={24} className="text-[#1a73e8]" /></div>
                                    <div className="flex-1">
                                        <h4 className="font-bold text-[#1a73e8] text-sm">{t('settings.local_backup')} ({backupSize})</h4>
                                        <div className="flex gap-2 mt-3">
                                            <Button variant="outline" size="sm" onClick={handleExportBackup}>{t('common.export')}</Button>
                                            <Button variant="outline" size="sm" onClick={() => fileInputRef.current?.click()}>{t('common.import')}</Button>
                                            <input type="file" ref={fileInputRef} className="hidden" accept=".json" onChange={handleFileSelect} />
                                        </div>
                                    </div>
                                </div>
                                <div className="bg-[#fef7e0] dark:bg-[#fbbc04]/10 p-6 rounded-none flex items-start gap-4">
                                    <div className="p-3 bg-white dark:bg-[#1a1c1e] rounded-none shadow-sm"><Cloud size={24} className="text-[#f9ab00]" /></div>
                                    <div className="flex-1">
                                        <h4 className="font-bold text-[#f9ab00] text-sm">Google Drive</h4>
                                        <div className="flex gap-2 mt-3">
                                            <Button variant="primary" size="sm" onClick={googleAccessToken ? handleCloudBackup : linkGoogleDrive}>{googleAccessToken ? t('settings.backup') : t('settings.connect')}</Button>
                                            {googleAccessToken && (
                                                <Button variant="ghost" size="sm" className="text-[#ea4335] hover:bg-red-50" onClick={unlinkGoogleDrive}>{t('settings.unlink')}</Button>
                                            )}
                                        </div>
                                    </div>
                                </div>
                                <div className="flex gap-2 pt-4 border-t border-[#f1f3f4]">
                                    <Button variant="secondary" className="flex-1" leftIcon={<Sparkles size={16} />} onClick={async () => { if (confirm(t('common.confirm'))) await generateDemoData(); window.location.reload(); }}>Demo</Button>
                                    <Button variant="ghost" className="flex-1 text-[#ea4335]" leftIcon={<Trash2 size={16} />} onClick={async () => { if (confirm(t('common.confirm'))) await cleanAllDuplicates(); }}>{t('common.clean')}</Button>
                                </div>
                                <div className="space-y-4">
                                    <h4 className="text-xs font-bold text-[#5f6368] uppercase">{t('settings.cloud_backups')}</h4>
                                    <div className="bg-[#f8f9fa] dark:bg-[#202124] rounded-none border border-[#dadce0] divide-y divide-[#dadce0] overflow-hidden max-h-60 overflow-y-auto">
                                        {isCloudLoading ? <p className="p-8 text-center text-xs">{t('common.loading')}</p> : cloudBackups.length === 0 ? <p className="p-8 text-center text-xs italic">{t('settings.empty_cloud')}</p> : cloudBackups.map(cb => (
                                            <div key={cb.url} className="px-5 py-3 flex items-center justify-between group hover:bg-white transition-colors">
                                                <div><p className="text-xs font-bold">{formatBackupDate(cb.date)}</p><p className="text-[10px] opacity-60">{cb.size}</p></div>
                                                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    <button onClick={() => handleRestoreFromCloud(cb.url)} className="p-1.5 text-[#1a73e8] hover:bg-[#e8f0fe] rounded-none"><RefreshCw size={14} /></button>
                                                    <button onClick={() => handleDeleteCloud(cb.url)} className="p-1.5 text-[#ea4335] hover:bg-[#fce8e6] rounded-none"><Trash2 size={14} /></button>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                ...
                <div className="pt-20">
                    ...
                    <Card className="border-red-100 dark:border-red-900/20 overflow-hidden" header={<div className="flex items-center gap-2 font-bold text-sm text-red-600"><AlertTriangle size={16} /> {t('settings.critical_data')}</div>}>
                        <div className="divide-y divide-red-50 dark:divide-red-900/10">
                            ...
                            <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div className="space-y-1">
                                    <p className="font-bold text-[#202124] dark:text-white text-sm">{t('settings.clear_sync')}</p>
                                    <p className="text-xs text-[#5f6368] dark:text-[#9aa0a6]">{t('settings.clear_sync_desc')}</p>
                                </div>
                                <Button
                                    variant="ghost"
                                    className="text-red-100 hover:text-red-100 border border-transparent hover:bg-red-500/10"
                                    onClick={() => {
                                        const keys = Object.keys(localStorage).filter(k => k.startsWith('lastSync_'));
                                        keys.forEach(k => localStorage.removeItem(k));
                                        toast.success("Caché de sincronización limpia");
                                    }}
                                >
